    ...
    public partial class ucJob_details : UserControl
    {
        ...
        private void jobTblaod() //load job details
        {
            try
            {
                ...
                DB.showRecords("SELECT JOB.Job_ID,JOB.Start_Date,JOB.End_Date,JOB.Start_Time,JOB.End_Time, JOB.Start_location, JOB.End_location,JOB.Job_Status, JOB.Cus_ID,JOB.Unit_ID, Customer.Cus_FullName, Customer.Cus_Status FROM JOB INNER JOIN Customer ON JOB.Cus_ID=Customer.Cus_ID", "JOB"); 
        ...
        {
            //check all fileds are fill
            if (txtUsr_id.Text == "" || txtStart_location.Text == "" || txtEnd_location.Text == "" || dtpStrtdate.Text == "" || dtpEnddate.Text == "" || cmbJob_status.Text == "" || dtpStartime.Text == "" || dtpEndtime.Text == "" || txtUnitID.Text == "")
            {
                //if not show a warning message
                MessageBox.Show("Please Enter All Fileds", "E-Shift", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            ...
            else //if all details are filled
            {
                try
                {
                    DB.openConnection(); //open sql connection
                    //insert record to database
                    DB.queryingRecord("insert into JOB values('" + dtpStrtdate.Value.ToString("yyyy-MM-dd") + "','" + dtpEnddate.Value.ToString("yyyy-MM-dd") + "', '" + dtpStartime.Value.ToString("h:mm tt") + "','" + dtpEndtime.Value.ToString("h:mm tt") + "','" + txtStart_location.Text + "', '" + txtEnd_location.Text + "', '" + cmbJob_status.Text + "', '" + txtUsr_id.Text + "', '"+txtUnitID.Text+"')");
                    //showing message box
                    MessageBox.Show("Successfully Registered.. Please Add product..!", "E-Shift", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                    clear_Textbox();//invoke clear textbox method
                    jobTblaod(); //invoke refresh datagrid view method
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (txtId.Text == "")
            {
                ...
                MessageBox.Show("Please Enter JOB ID", "E-Shift", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                try
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                //then show error message
                MessageBox.Show("Please choose JOB ID or Customer ID", "E-Shift", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else if(txtId.Text == "")
            {
                // then show error message
                MessageBox.Show("Please Enter ID", "E-Shift", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        private void dgvJob_tbl_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                txtStart_location.Text = row.Cells["Start_location"].Value.ToString();
                txtEnd_location.Text = row.Cells["End_location"].Value.ToString();
                cmbJob_status.Text = row.Cells["Job_Status"].Value.ToString();
                txtUsr_id.Text = row.Cells["Cus_ID"].Value.ToString();
                txtCus_Name.Text = row.Cells["Cus_FullName"].Value.ToString();
                txtCus_status.Text = row.Cells["Cus_Status"].Value.ToString();
                txtUnitID.Text = row.Cells["Unit_ID"].Value.ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error : " + ex.Message, "E-Shift", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void btnAll_jobs_Click(object sender, EventArgs e)
        {
            frmAll_jobs all_Jobs = new frmAll_jobs();
            all_Jobs.Show();
        }
